...
LangGraph.js 기반의 에이전틱 플로우. `lib/generator.ts`에서 호출하는 **4-노드 + 조건부 분기 + 1회 retry** 그래프입니다.
...
| `verifier` | `nodes/verifier.ts` | Upstage Information Extract(`information-extract` 모델) 호출로 카드 이미지에서 storeName/dish/benefit/koreanText 추출 → 입력 키워드와 정합성 검증. Mock 분기는 검증 생략. |
